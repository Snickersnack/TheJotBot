package org.wilson.theJotBot;

/**
 * Static list of available commands
 * 
 */

public class Commands {
    public static final String commandInitChar = "/";
//    public static final String TASKCOMMAND =  "task";
//    public static final String TASKSCOMMAND = "tasks";
    public static final String REMINDCOMMAND = commandInitChar + "remind";
    public static final String JOTCOMMAND = commandInitChar +"jot";
    public static final String JOTSCOMMAND = commandInitChar +"jots";
    public static final String SKIPCOMMAND = commandInitChar + "skip";
    public static final String HELPCOMMAND = commandInitChar + "help";




    



}
